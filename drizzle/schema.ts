import { mysqlTable, mysqlSchema, AnyMySqlColumn, varchar, datetime, tinyint, index, foreignKey, text, binary, uniqueIndex, decimal, int } from "drizzle-orm/mysql-core"
...
export const bands = mysqlTable("BANDS", {
	...
	stringId: int("string_id").notNull().references(() => bandStringRegister.stringId, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const capture = mysqlTable("CAPTURE", {
	...
	netEffId: int("net_eff_id").notNull().references(() => netEffort.netEffId, { onDelete: "restrict", onUpdate: "restrict" } ),
	banderId: int("bander_id").notNull().references(() => banderRegister.banderId, { onDelete: "restrict", onUpdate: "restrict" } ),
	bandId: int("band_id").notNull().references(() => bands.bandId, { onDelete: "restrict", onUpdate: "restrict" } ),
	sppId: int("spp_id").notNull().references(() => sppRegister.sppId, { onDelete: "restrict", onUpdate: "restrict" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}),
	originalId: int("original_id"),
...
export const captureCategoricalOptions = mysqlTable("CAPTURE_CATEGORICAL_OPTIONS", {
	captureCategoricalOptionId: int("capture_categorical_option_id").autoincrement().primaryKey().notNull(),
	description: text("description").notNull(),
	valueOama: varchar("value_oama", { length: 45 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	originalId: int("original_id"),
	captureVariableId: int("capture_variable_id").notNull().references(() => captureVariableRegister.captureVariableId, { onDelete: "restrict", onUpdate: "restrict" } ),
	updatedAt: datetime("updated_at", { mode: 'string'}),
},
(table) => {
	return {
		captureVariableId: index("capture_variable_id").on(table.captureVariableId),
	}
});

export const captureCategoricalValues = mysqlTable("CAPTURE_CATEGORICAL_VALUES", {
	captureCategoricalValuesId: int("capture_categorical_values_id").autoincrement().primaryKey().notNull(),
	captureId: int("capture_id").notNull().references(() => capture.captureId, { onDelete: "restrict", onUpdate: "restrict" } ),
	captureCategoricalOptionId: int("capture_categorical_option_id").notNull().references(() => captureCategoricalOptions.captureCategoricalOptionId, { onDelete: "restrict", onUpdate: "restrict" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	originalId: int("original_id"),
	updatedAt: datetime("updated_at", { mode: 'string'}),
...
(table) => {
	return {
		...
		captureCategoricalOptionId: index("capture_categorical_option_id").on(table.captureCategoricalOptionId),
		captureVariableIdForeignIdx: index("CAPTURE_CATEGORICAL_VALUES_capture_variable_id_foreign_idx").on(table.captureVariableId),
	}
});

export const captureContinuousValues = mysqlTable("CAPTURE_CONTINUOUS_VALUES", {
	captureContinuousValuesId: int("capture_continuous_values_id").autoincrement().primaryKey().notNull(),
	captureId: int("capture_id").notNull().references(() => capture.captureId, { onDelete: "restrict", onUpdate: "restrict" } ),
...
(table) => {
	return {
		captureId: index("capture_id").on(table.captureId),
		captureVariableIdForeignIdx: index("CAPTURE_CONTINUOUS_VALUES_capture_variable_id_foreign_idx").on(table.captureVariableId),
	}
});

export const captureFlag = mysqlTable("CAPTURE_FLAG", {
...
export const effort = mysqlTable("EFFORT", {
	...
	originalId: int("original_id"),
	stationId: int("station_id").notNull().references(() => stationRegister.stationId, { onDelete: "restrict", onUpdate: "restrict" } ),
	protocolId: int("protocol_id").notNull().references(() => protocolRegister.protocolId, { onDelete: "restrict", onUpdate: "restrict" } ),
	updatedAt: datetime("updated_at", { mode: 'string'}),
},
(table) => {
	return {
...
export const effortCategoricalOptions = mysqlTable("EFFORT_CATEGORICAL_OPTIONS", {
	effortCategoricalOptionId: int("effort_categorical_option_id").autoincrement().primaryKey().notNull(),
	description: text("description").notNull(),
	valueOama: varchar("value_oama", { length: 45 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	originalId: int("original_id"),
	effortVariableId: int("effort_variable_id").notNull().references(() => effortVariableRegister.effortVariableId, { onDelete: "restrict", onUpdate: "restrict" } ),
	updatedAt: datetime("updated_at", { mode: 'string'}),
},
(table) => {
	return {
...
export const effortCategoricalValues = mysqlTable("EFFORT_CATEGORICAL_VALUES", {
	effortCategoricalValueId: int("effort_categorical_value_id").autoincrement().primaryKey().notNull(),
	effortId: int("effort_id").notNull().references(() => effort.effortId, { onDelete: "restrict", onUpdate: "restrict" } ),
	effortCategoricalOptionId: int("effort_categorical_option_id").notNull().references(() => effortCategoricalOptions.effortCategoricalOptionId, { onDelete: "restrict", onUpdate: "restrict" } ),
	effortTimeId: int("effort_time_id").notNull().references(() => effortTime.effortTimeId, { onDelete: "restrict", onUpdate: "restrict" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	originalId: int("original_id"),
	updatedAt: datetime("updated_at", { mode: 'string'}),
...
(table) => {
	return {
		...
		effortCategoricalOptionId: index("effort_categorical_option_id").on(table.effortCategoricalOptionId),
		effortTimeId: index("effort_time_id").on(table.effortTimeId),
		effortVariableIdForeignIdx: index("EFFORT_CATEGORICAL_VALUES_effort_variable_id_foreign_idx").on(table.effortVariableId),
	}
});

export const effortContinuousValues = mysqlTable("EFFORT_CONTINUOUS_VALUES", {
	effortContinuousValueId: int("effort_continuous_value_id").autoincrement().primaryKey().notNull(),
	effortId: int("effort_id").notNull().references(() => effort.effortId, { onDelete: "restrict", onUpdate: "restrict" } ),
...
(table) => {
	return {
		effortId: index("effort_id").on(table.effortId),
		effortTimeId: index("effort_time_id").on(table.effortTimeId),
		effortVariableIdForeignIdx: index("EFFORT_CONTINUOUS_VALUES_effort_variable_id_foreign_idx").on(table.effortVariableId),
	}
});

export const effortSummaries = mysqlTable("EFFORT_SUMMARIES", {
	effortSummaryId: int("effort_summary_id").autoincrement().primaryKey().notNull(),
	effortId: int("effort_id").notNull().references(() => effort.effortId, { onDelete: "restrict", onUpdate: "restrict" } ),
...
export const netEffort = mysqlTable("NET_EFFORT", {
	netEffId: int("net_eff_id").autoincrement().primaryKey().notNull(),
	effortId: int("effort_id").notNull().references(() => effort.effortId, { onDelete: "restrict", onUpdate: "restrict" } ),
	netId: int("net_id").notNull().references(() => netRegister.netId, { onDelete: "restrict", onUpdate: "restrict" } ),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	hasChanged: tinyint("has_changed").notNull(),
	originalId: int("original_id"),
	updatedAt: datetime("updated_at", { mode: 'string'}),
...
export const netRegister = mysqlTable("NET_REGISTER", {
...
export const protocolVars = mysqlTable("PROTOCOL_VARS", {
	protocolParamId: int("protocol_param_id").autoincrement().primaryKey().notNull(),
	protocolId: int("protocol_id").notNull().references(() => protocolRegister.protocolId, { onDelete: "restrict", onUpdate: "restrict" } ),
