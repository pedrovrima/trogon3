...
import db from "@/db";
import { bandStringRegister, bands, capture, effort, netEffort, sppRegister, stationRegister } from "drizzle/schema";
  ...
  hello: publicProcedure
    ...
    .query(async ({ input }) => {
            ...
            .select({
                ...
                speciesName: sql<string|null>`concat(${sppRegister.genus}," ",${sppRegister.species})`,
                date: effort.dateEffort, station: stationRegister.stationCode,
            })
            .from(bands)
            .leftJoin(capture, eq(bands.bandId, capture.bandId))
            .rightJoin(bandStringRegister, eq(bands.stringId, bandStringRegister.stringId))
            ...
            .where(and(eq(bands.bandNumber, bandNumber as string),eq(bandStringRegister.size, bandSize as string)))
